        ...
        try {
          ...
          for (const teamId of teamIdsToQuery) {
            try {
              ...
              const annotateTeam = (arr: any[] = []) => arr.map((item) => ({ ...item, team_id: item.team_id ?? (teamId || null) }));
          ...
          const uniqueUsers = Array.from(
            new Map(
              [...aggregatedWlog, ...aggregatedVacation, ...aggregatedEvent].map((w) => [
                w.user_id,
                { user_id: w.user_id, user_name: w.user_nm, team_id: w.team_id },
              ])
            ).values()
          );
          ...
          for (const userInfo of uniqueUsers) {
            ...
            const formatDayTime = (dayData: WorkData): DayWorkInfo => {
              ...
              if (dayData.overtimeStatus === '승인완료') {
                const totalTime =
                  dayData.totalHours === 0 && dayData.totalMinutes === 0 ? '진행중' : `${dayData.totalHours}h ${dayData.totalMinutes}m`;
