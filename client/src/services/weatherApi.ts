...
class WeatherApiService {
  ...
  async getCurrentWeather(): Promise<Weather | null> {
    ...
    try {
      ...
      // 두 API를 병렬로 호출
      const [ncstResponse, fcstResponse] = await Promise.all([fetch(`${ncstUrl}?${ncstParams}`), fetch(`${fcstUrl}?${fcstParams}`)]);
...
function saveCachedWeather(data: Weather) {
  ...
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({ expiresAt: Date.now() + CACHE_TTL_MS, data }));
  } catch (err) {
