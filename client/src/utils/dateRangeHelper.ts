...
export const getDateRangeTextSimple = (startDateStr?: string, endDateStr?: string, startTime?: string, endTime?: string, allDay?: boolean) => {
  ...
  // 시간 포함
  if (startDate.isSame(endDate, 'day')) {
    return `${startDate.format('YYYY-MM-DD')} ${formatTime(startTime)} - ${formatTime(endTime)}`;
  }
  return `${startDate.format('YYYY-MM-DD')} ${formatTime(startTime)} - ${endDate.format('YYYY-MM-DD')} ${formatTime(endTime)}`;
};
