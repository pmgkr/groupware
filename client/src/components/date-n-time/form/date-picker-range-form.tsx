...
export function DatePickerWithRangeForm({
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    toast.success(
      `선택된 날짜 범위: ${format(data.dateRange.from, 'yyyy년 M월 d일 EEEE', { locale: ko })} ~ ${format(data.dateRange.to, 'yyyy년 M월 d일 EEEE', { locale: ko })}`
    );
  }
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <FormDescription>이벤트의 날짜 범위를 선택해주세요.</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
