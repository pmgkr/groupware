...
export const PExpenseCard = memo(({ role, item, activeTab, checked, onCheck, onAInfo }: Props) => {
  ...
  const showCheckbox = role === 'admin' || activeTab === 'saved' || activeTab === 'claimed';
  const canCheck =
    role === 'admin'
      ? item.status !== 'Saved' && item.status !== 'Rejected'
      : role === 'manager'
      ? item.status === 'Claimed'
      : (activeTab === 'saved' && item.user_id === user_id) || (activeTab === 'claimed' && item.status === 'Claimed');

  const detailUrl =
    role === 'admin'
      ? `/admin/finance/pexpense/${item.seq}${search}`
      : role === 'manager'
      ? `/manager/pexpense/${item.seq}${search}`
...
PExpenseCard.displayName = 'PExpenseCard';
