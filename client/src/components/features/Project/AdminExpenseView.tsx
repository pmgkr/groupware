...
import { formatAmount, sanitizeFilename, formatDate, normalizeAttachmentUrl } from '@/utils';
          ...
          <div className="-mx-4.5 -my-2 bg-white">
            ...
            <div className="p-5">
              ...
              <div className="flex gap-2">
                <Button variant="outline" asChild className={cn(isActionable ? 'flex-1' : 'w-full')}>
                  <Link to={`/admin/finance${search}`}>목록</Link>
                </Button>

                {isActionable && (
                  <>
                    <Button type="button" variant="destructive" className="flex-1" onClick={() => setReasonDialogOpen(true)}>
                      반려하기
                    </Button>
      ...
      <Dialog open={reasonDialogOpen} onOpenChange={setReasonDialogOpen}>
        <DialogContent className="max-w-sm max-md:max-w-[calc(100%-var(--spacing)*8)] max-md:rounded-md">
          ...
          <DialogFooter className="justify-center max-md:flex-row-reverse max-md:gap-2">
            <Button type="button" variant="outline" onClick={() => setReasonDialogOpen(false)} className="max-md:flex-1">
              취소
            </Button>
