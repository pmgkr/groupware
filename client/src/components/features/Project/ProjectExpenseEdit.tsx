...
export default function ProjectExpenseEdit() {
  ...
  const handleFilesChange = useCallback((newFiles: PreviewFile[]) => {
    setFiles(newFiles);
  ...
  const handleOpenMatchingDialog = async () => {
    ...
    try {
      // 응답 구조: { ok: boolean, items: ProposalItem[] }
  ...
  const handleConfirmSubmit = async (values: EditFormValues) => {
    ...
    try {
      ...
      if (allNewFiles.length > 0) {
        ...
        const totalSteps = chunks.length + 1; // +1 은 projectExpenseUpdate 호출
            ...
            '<p class="text-2xl text-primary leading-[1.3] font-bold mb-2">증빙자료 업로드 실패</p><p>서버 오류로 증빙자료 파일 업로드에 실패했습니다.\n증빙자료 없이 수정을 계속하시겠습니까, 아니면 취소하시겠습니까?</p>',
            {
              actions: [
                ...
                },
                {
                  label: '비용 수정 취소',
                  onClick: () => {
                    navigate(`/project/${projectId}/expense/${expId}`);
                  },
                },
              ],
            }
      ...
      if (res.ok) {
          ...
          try {
            ...
            const matchPromises = enrichedItems
              ...
              .map(async ({ pro_id, item_seq }) => {
                const matchResult = (await matchProjectWithProposal(pro_id as number, item_seq as number)) as {
                  success: boolean;
                  result: { type: string };
                };
        ...
        await showResult(
          'success',
          `<p class="text-2xl text-primary leading-[1.3] font-bold mb-2">비용 수정 완료</p><p><span class="text-primary-blue-500 font-semibold">${res.updated.itemCount}건</span>의 프로젝트 비용이 수정되었습니다.</p>`
        );
                      ...
                      <div className="flex flex-col justify-between gap-2 md:flex-row md:flex-wrap md:gap-0">
                        ...
                        <div className="w-full max-md:order-3 md:w-[32%] md:pl-2">
                          <AttachmentFieldEdit
                            ...
                            rowIndex={index + 1}
                            files={[
                              ...(rowAttachments[index + 1]?.map((att) => ({ name: att.fname, type: 'image', preview: att.ea_url })) ?? []),
