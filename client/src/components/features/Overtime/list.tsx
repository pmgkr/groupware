...
export default function OvertimeList({
    ...
    async (force = false) => {
      ...
      try {
        ...
        if (isPage === 'admin') {
          ...
          const responses =
            teamIdsToQuery.length === 0
              ? [await adminOvertimeApi.getOvertimeList(undefined, 1, 1000, '')]
              : await Promise.all(teamIdsToQuery.map((teamId) => adminOvertimeApi.getOvertimeList(teamId, 1, 1000, '')));
          ...
          const uniqueItems = convertedItems.filter((item, index, self) => index === self.findIndex((t) => t.id === item.id));

          setAllData(uniqueItems);
        } else {
          // Manager API 사용
          // teamIds가 비어있으면 (필터 미선택), 관리중인 모든 팀의 데이터를 각각 조회하여 병합
          const targetTeamIds = teamIdsToQuery.length > 0 ? teamIdsToQuery : teams.map((t) => t.team_id);
  ...
  const handleCheckAll = (checked: boolean) => {
    setCheckAll(checked);
    const newCheckedItems = checked ? paginatedData.filter((item) => isSelectableStatus(item.ot_status)).map((item) => item.id) : [];
  ...
  // 신청자에게 알림 전송
  const notifyApplicant = async (item: OvertimeItem | overtimeItem | null | undefined, kind: 'approve' | 'compensation' | 'reject') => {
  ...
  const handleApproveAllOvertime = () => {
    ...
    // 체크된 항목들을 상태별로 분류
    const checkedOvertimeItems = allData.filter((item) => checkedItems.includes(item.id) && item.ot_status === 'H');
    const checkedCompensationItems = allData.filter((item) => checkedItems.includes(item.id) && item.ot_status === 'T');
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      // 모든 체크된 항목에 대해 승인 요청
      if (isPage === 'admin') {
        await Promise.all(checkedItems.map((id) => adminOvertimeApi.approveOvertime(id)));
      } else {
        await Promise.all(checkedItems.map((id) => managerOvertimeApi.approveOvertime(id)));
      }
      // 알림 전송 (신청자에게만)
  ...
  return (
    <>
      <Table variant="primary" align="center" className={cn(isMobileNoData ? 'table-auto' : 'table-fixed')}>
        <TableHeader>
          <TableRow className="[&_th]:text-[13px] [&_th]:font-medium">
            <TableHead
              className={cn(
                'max-md:w-unset! w-[7%] p-2 text-center max-md:hidden max-md:max-w-[50px]! max-md:px-0.5 max-md:text-sm!',
            ...
            <TableHead
              className={cn(
                'max-md:w-unset! w-[7%] p-2 text-center max-md:max-w-[50px]! max-md:px-0.5 max-md:text-sm!',
                isMobileNoData && 'w-auto'
              )}>
              이름
            </TableHead>
            ...
            {activeTab === 'weekday' ? (
              <>
                <TableHead
                  className={cn(
                    'max-md:w-unset! w-[8%] p-2 text-center max-md:min-w-[90px]! max-md:px-0.5 max-md:text-sm!',
                    isMobileNoData && 'w-auto'
                  )}>
                  예상퇴근시간
                </TableHead>
                {!isMobile && <TableHead className="w-[8%] p-2 text-center max-md:px-0.5 max-md:text-sm!">식대</TableHead>}
                {!isMobile && <TableHead className="w-[8%] p-2 text-center max-md:px-0.5 max-md:text-sm!">교통비</TableHead>}
              </>
            ) : (
              <>
                <TableHead className={cn('w-[16%] p-2 text-center max-md:px-0.5 max-md:text-sm!', isMobileNoData && 'w-auto')}>
                  예상근무시간
                </TableHead>
                <TableHead className={cn('w-[10%] p-2 text-center max-md:px-0.5 max-md:text-sm!', isMobileNoData && 'w-auto')}>
                  보상방식
                </TableHead>
              </>
            )}
            {!isMobile && <TableHead className="w-[12%] p-2 text-center max-md:px-0.5 max-md:text-sm!">클라이언트명</TableHead>}
            {!isMobile && <TableHead className="w-[20%] p-2 text-center max-md:px-0.5 max-md:text-sm!">작업내용</TableHead>}
            {!isMobile && <TableHead className="w-[10%] p-2 text-center max-md:px-0.5 max-md:text-sm!">신청일</TableHead>}
            <TableHead className={cn('w-[10%] p-2 text-center max-md:px-0.5 max-md:text-sm!', isMobileNoData && 'w-auto')}>상태</TableHead>
            {!(isPage === 'admin' && activeTab === 'weekday') && (
              <TableHead className={cn('w-[5%] p-2 text-center max-md:px-0.5', isMobileNoData && 'w-auto')}>
                <Checkbox
                  id="chk_all"
          ...
          {loading ? (
            ...
            paginatedData.map((item) => (
              <TableRow
                ...
                <TableCell className="max-md:w-unset! overflow-hidden p-2 text-center text-ellipsis whitespace-nowrap max-md:hidden max-md:max-w-[50px]! max-md:px-0.5">
                  {getTeamName(item.team_id)}
                </TableCell>
                <TableCell className="max-md:w-unset! p-2 text-center max-md:max-w-[50px]! max-md:px-0.5">{item.user_name}</TableCell>
                <TableCell className="max-md:w-unset! p-2 text-center whitespace-nowrap max-md:min-w-[90px]! max-md:px-0.5">
                  {item.ot_date ? dayjs(item.ot_date).format(isMobile ? 'MM-DD' : 'YYYY-MM-DD (ddd)') : '-'}
                </TableCell>
                {activeTab === 'weekday' ? (
                  <>
                    <TableCell className="w-[8%] p-2 text-center max-md:px-0.5">{formatTime(item.ot_etime)}</TableCell>
                    {!isMobile && <TableCell className="w-[8%] p-2 text-center">{getYNText(item.ot_food)}</TableCell>}
                    {!isMobile && <TableCell className="w-[8%] p-2 text-center max-md:px-0.5">{getYNText(item.ot_trans)}</TableCell>}
                  </>
                ) : (
                  <>
                    <TableCell className="max-md:w-unset! w-[16%] p-2 text-center max-md:flex max-md:min-w-[90px]! max-md:flex-col max-md:gap-0 max-md:px-0.5">
                      {formatTime(item.ot_stime)} - {formatTime(item.ot_etime)}{' '}
                      <span className="text-sm font-normal text-gray-500">({formatHours(item.ot_hours)})</span>
                    </TableCell>
                    <TableCell className="w-[10%] p-2 text-center max-md:px-0.5">{getRewardText(item.ot_reward)}</TableCell>
                  </>
                )}
                {!isMobile && (
                  <TableCell className="overflow-hidden p-2 text-center text-ellipsis whitespace-nowrap max-md:px-0.5">
                    {item.ot_client || '-'}
                  </TableCell>
                )}
                {!isMobile && (
                  <TableCell className="overflow-hidden p-2 text-left text-ellipsis whitespace-nowrap max-md:px-0.5">
                    {item.ot_description || '-'}
                  </TableCell>
                )}
                {!isMobile && (
                  <TableCell className="p-2 text-center whitespace-nowrap max-md:px-0.5">
                ...
                {!(isPage === 'admin' && activeTab === 'weekday') && (
                  <TableCell className="p-2 text-center max-md:px-0.5" onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      id={`chk_${item.id}`}
                      className={cn(
        ...
        (() => {
          ...
          return (
            <OvertimeViewDialog
              ...
              onCompensation={isManager && isPage === 'admin' && activeTab === 'weekend' ? handleCompensationOvertime : undefined}
          ...
          const selectedDay = {
            ...
            overtimeData: overtimeDetailData?.info
              ? {
                  ...
                  overtimeMinutes: overtimeDetailData.info.ot_hours
                    ? String(Math.round((parseFloat(overtimeDetailData.info.ot_hours) % 1) * 60))
                    : '',
                  overtimeType:
                    overtimeDetailData.info.ot_reward === 'special'
                      ? 'special_vacation'
                      : overtimeDetailData.info.ot_reward === 'annual'
      ...
      <AlertDialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{isPage === 'admin' && activeTab === 'weekend' ? '보상 지급 확인' : '승인 확인'}</AlertDialogTitle>
            <AlertDialogDescription>
              {(() => {
                if (isPage === 'manager') {
                  return <>{approveCounts.overtime}개의 연장근무 요청을 승인하시겠습니까?</>;
                  ...
                  return <>{totalCount}개의 연장근무 요청을 승인하시겠습니까?</>;
                }
              })()}
            </AlertDialogDescription>
          </AlertDialogHeader>
